"""
Export a Mixamo-rigged humanoid FBX or GLB as GLB with its skeleton rest baked to
...
Args via env vars:
  MESH_FBX   absolute path to the input Mixamo FBX or GLB (T-pose action)
...
def main():
    ...
    is_glb = MESH_FBX.lower().endswith('.glb') or MESH_FBX.lower().endswith('.gltf')
    if is_glb:
        bpy.ops.import_scene.gltf(filepath=MESH_FBX)
    ...
    if not meshes or not rig:
        print('ERROR: source missing mesh or armature')
        sys.exit(1)

    print(f"meshes: {[m.name for m in meshes]}, armature: {rig.name}")
    ...
    # Re-parent meshes to armature so the export writes skin bindings.
    for o in bpy.data.objects:
        try:
            o.select_set(False)
        except RuntimeError:
